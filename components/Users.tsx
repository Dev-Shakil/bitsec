...
//           <div className="absolute top-full mt-1 w-full bg-white border rounded shadow-md max-h-40 overflow-y-auto z-10">
//             {suggestions.length > 0 ? (
//               suggestions.map((user) => (
//                 <div
//                   key={user.id}
//                   onClick={() => {
//                     setQuery(user.email); // pick email (can change to user.name if needed)
//                     setShowSuggestions(false);
//                   }}
...
//               ))
//             ) : (
//               <p className="px-3 py-2 text-gray-500 text-sm">No matches found</p>
//             )}
//           </div>
//         )}
//       </form>
...
export default function Users({ users }: { users: User[] }) {
  ...
  return (
    <div className="px-8 py-4 rounded-xl bg-white min-h-screen">
      ...
      <motion.div
        key={currentPage} // re-animates on page change
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
